...
let simpl_block (block:program) newProgram :program=
  ...
  match block with 
    ...
    |(pos,ins)::r-> match ins with 
        ...
        | Read name -> simpl_ins r ((pos,ins)::newProgram)
        | Print expr ->simpl_ins r ((pos,Print (simpl_exp expr))::newProgram)
        | If(cond,block,block') ->begin match simpl_cond cond  with 
          |(Num x,comp,Num y) -> 
              if eval_comp comp (Num x) (Num y) 
              then simpl_ins r (List.rev_append (simpl_ins block []) newProgram)
              else simpl_ins r (List.rev_append (simpl_ins block' []) newProgram)
          | (e1, comp, e2) -> 
              let simplified_if = If((e1, comp, e2), simpl_ins block [], simpl_ins block' []) in
              simpl_ins r ((pos, simplified_if) :: newProgram)
        end
        | While(cond,block) ->begin match simpl_cond cond with 
          |(Num x,comp,Num y) ->
              if eval_comp comp (Num x) (Num y)
              then simpl_ins r ((pos,While((Num x,comp,Num y),List.rev(simpl_ins block [])))::newProgram)
              else simpl_ins r newProgram
          |(e1,comp,e2)->simpl_ins r ((pos,While(simpl_cond cond,simpl_ins block []))::newProgram)
        end 
